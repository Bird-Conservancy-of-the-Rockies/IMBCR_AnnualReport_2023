...
$$\begin{equation}z_{ijt} \sim Bernoulli(ψ_{it})\end{equation}$$ {#eq-hurdle_z}

<center>and</center>

$$\begin{equation}logit(ψ_{it}) \sim Normal(μ_{[ψ],t},SD_{[ψ],t}^{2})\end{equation}$$ {#eq-hurdle_psi}
...
$$\begin{equation}u_{ijt}|z_{ijt} \sim Binomial(K,θ_{it}×z_{ijt})\end{equation}$$ {#eq-hurdle_u}

<center>and</center>

$$\begin{equation}logit(θ_{it}) \sim Normal(μ_{[θ],t},SD_{[θ],t}^{2}),\end{equation}$$ {#eq-hurdle_theta}
...
$$\begin{equation}N_{ijt} \sim Poisson(λ_{ijt}×u_{ijt})+u_{ijt}\end{equation}$$ {#eq-hurdle_N}

<center>and</center>

$$\begin{equation}log(λ_{ijt})=α_{it}+ε_{jt}\end{equation}$$ {#eq-hurdle_lambda}
...
$$\begin{equation}α_{it} \sim Normal(μ_{[α],t},SD_{[α],t}^{2})\end{equation}$$ {#eq-hurdle_alpha}

<center>and</center>

$$\begin{equation}ε_{it} \sim Normal(0,SD_{[ε]}^{2}).\end{equation}$$ {#eq-hurdle_epsilon}

We derive density, D, as

$$\begin{equation}D_{ijt}= \frac{N_{ijt}×s}{K×∑_{d=1}^{d_{[max]}}A_{d}},\end{equation}$$ {#eq-hurdle_D}
...
$$\begin{equation}s \sim Gamma(k,v)+1,\end{equation}$$ {#eq-hurdle_s}
...
$$\begin{equation}n_{ijtl} \sim Binomial\biggl(N_{ijt},p_{[a],tl}×p_{[p],t}×\biggl(\frac{K_{[s],ijt}}{K}\biggr)\biggr),\end{equation}$$ {#eq-hurdle_n}

$$\begin{equation}y_{ijtl} \sim Binomial\biggl(u_{ijt},p_{[s],tl}×\biggl(\frac{K_{[s],ijt}}{K}\biggr)\biggr)\end{equation},~\mathrm{and}$$ {#eq-hurdle_y}

$$\begin{equation}p_{[s],tl}=1-(1-p_{[a],tl}×p_{[p],t})^{N_{ijt}}\end{equation},$$ {#eq-hurdle_p_s}
...
$$\begin{equation}p_{[a],tl}=∑_{r=1}^{R} \mathbf{π_{[a],tl}} \end{equation}$$ {#eq-hurdle_p_a}

<center>and</center>

$$\begin{equation}p_{[p],tl}=∑_{d=1}^{d_{max}} \mathbf{π_{[p],tl}} \end{equation},$$ {#eq-hurdle_p_p}

consisting of multinomial cell probabilities for

$$\begin{equation}\mathbf{y_{[a],ijtl}} \sim Multinomial \biggl(n_{ijt},\mathbf{π_{[a],tl}^{[c]}} \biggr)\end{equation}$$ {#eq-hurdle_pi_a}

<center>and</center>

$$\begin{equation}\mathbf{y_{[p],ijtl}} \sim Multinomial \biggl(n_{ijt},\mathbf{π_{[p],tl}^{[c]}} \biggr)\end{equation},$$ {#eq-hurdle_pi_p}

where $\mathbf{y_{[a],ijtl}}$ and $\mathbf{y_{[p],ijt}}$ are vectors of counts within time removal and distance bins, respectively. Cell probabilities for availability assume constant availability across the survey period, $$\begin{equation}π_{[a],trl}=p_{[ar],tl} (1-p_{[ar],tl})^{r-1}\end{equation},$$ {#eq-hurdle_pi_a_cell} and are scaled to sum to 1,

$$\begin{equation}π_{[a],trl}^{[c]}=π_{[a],trl}⁄p_{[a],tl} \end{equation},$$ {#eq-hurdle_pi_a_cell_c}
...
$$\begin{equation}π_{[p],td}=\frac{p_{[pd],td}A_d}{∑_{d=1}^{d_{[max]}}A_d}\end{equation},~\mathrm{and}$$ {#eq-hurdle_pi_p_cell}

$$\begin{equation}π_{[p],td}^{[c]}=π_{[p],td}⁄p_{[p],t}\end{equation},$$ {#eq-hurdle_pi_p_cell_c}
...
$$\begin{equation}p_{[pd],td}=\frac{2\pi\int_{c=d_{[low]}}^{c=d_{[high]}}exp\biggl(-\biggl(\frac{c^2}{2\sigma_{t^2}}\biggr)\biggr)c~dc}{A_d} \end{equation},$$ {#eq-hurdle_p_pd_HN}
...
$$\begin{equation}p_{[pd],td}=1-exp\biggl(-\biggl(\frac{d_{[mid]}}{h_t}\biggr)^b\biggr)\end{equation},$$ {#eq-hurdle_p_pd_HZ}

where $h_t$ and $b$ are estimated parameters for the hazard function. For perceptibility models $\eqref{#eq-hurdle_p_pd_HN}$ and $\@eqref{#eq-hurdle_p_pd_HZ}$, we draw year-specific shape parameters from hyperdistributions:
